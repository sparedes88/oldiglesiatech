...
export class DetailsComponent implements OnInit {
  ...
  async getTimes() {
    const response: any = await this.api.get('test_timezone', {}).toPromise();
    if (response) {
      this.test_times = response;
    }
    const response_prod: any = await this.api.http.get(`${prod.apiUrl}/test_timezone`, {}).toPromise();
    if (response_prod) {
  ...
  getMailingList() {
    this.loading = true;
    this.api
      .get(`mailingList/${this.mailingListId}`, {
  ...
  getContacts() {
    this.api.get(`mailingList/${this.mailingListId}/contacts`).subscribe(
      (data: any) => {
        this.contacts = data;
...
`,
  ...
  updateUser(event) {
    ...
    this.api.patch(`mailingList/${this.mailingListId}/contacts/${event.contact.id}/accept`, {}).subscribe(
      (data: any) => {
        this.restartTable();
        this.ngOnInit();
      },
  ...
  updateStatus(contact) {
    ...
    this.api.patch(`mailingList/${this.mailingListId}/contacts/${contact.id}/status/`, contact)
      .subscribe(
        (data: any) => {
  ...
  updateUserField(event, contact, key_field: string) {
    ...
    this.api.patch(`mailingList/${this.mailingListId}/contacts/${contact.id}/update/${key_field}`, contact)
      .subscribe(
        (data: any) => {
          this.toastr.success('Contact updated successfully!', 'Success');
          this.restartTable();
  ...
  deleteContact(contact_inbox) {
    if (confirm(`Are you sure you want to delete this contact? \n This action can't be undone`)) {
      this.api.delete(`mailingList/${contact_inbox.idMailingList}/contacts/${contact_inbox.id}?deleted_by=${this.currentUser.idUsuario}`)
        .subscribe(response => {
  ...
  setStatus(contact, i: number, status) {
    ...
    this.api.patch(`mailingList/${this.mailingListId}/contacts/${contact.id}/status/`, payload)
      .subscribe(
        (data: any) => {
          this.toastr.success('Contact updated successfully!', 'Success');
          this.restartTable();
    ...
    if (this.view_columns) {
      ...
      if (this.mailingList) {
        if (this.mailingList.setup) {
          this.mailingList.setup.hide_email_input ? null : columns_count++;
  ...
  openLinkInBrowser(language: 'en' | 'es') {
    const win = window.open(`${this.embedCode.link_without_lang}?lang=${language}`, '_blank');
    win.focus();
  }
  ...
  async printReport() {
    ...
    const response = await this.api.post(`mailingList/${this.mailingListId}/filter/contacts/pdf`, settings, {
      responseType: 'arraybuffer'
    }).toPromise()
      .catch(error => {
        console.error(error);
        this.api.showToast(`Error downloading the report`, ToastType.error);
