...
@Component({
  selector: 'app-public-design-home',
  templateUrl: './public-design-home.component.html',
  styleUrls: ['./public-design-home.component.scss']
})
export class PublicDesignHomeComponent implements OnInit {
  ...
  load() {
    ...
    Promise.all(promises).then((response: any) => {
      ...
      if (query_params.status) {
        ...
        console.log(min_status_summary);
        if (min_status_summary) {
          ...
          this.getRequestByStatus(this.statuses[0]);
        }
      } else {
        const status_id = this.statuses.filter(x => x.summary > 0).map(x => x.summary);
        const min = status_id.length > 0 ? Math.min(...status_id) : 0;
  ...
  initDesignRequest(testing_form: PublicDesignFormComponent) {
    if (!testing_form.isEdit) {
      testing_form.designRequest = new DesignRequestModel();
      testing_form.ngOnInit();
    }
  }

  async getStatuses() {
    return this.designRequestService.getStatus({
  ...
  onModalDidDismiss(response?: any) {
    this.modal.getModal('design_request_form_modal_public').close();
    if (response) {
      this.resetTable();
      this.load();
    }
  ...
  addDesignRequest(testing_form: PublicDesignFormComponent) {
    this.modal.getModal('design_request_form_modal_public').open();
    testing_form.designRequest = new DesignRequestModel();
    testing_form.isEdit = false;
    testing_form.ngOnInit();
  }

  openEditModal(item: DesignRequestModel, testing_form: PublicDesignFormComponent) {
    testing_form.isEdit = true;
    testing_form.ngOnInit();
    if (item.deadline) {
    ...
    testing_form.designRequest = Object.assign({}, item);
    this.modal.getModal('design_request_form_modal_public').open();
  }
