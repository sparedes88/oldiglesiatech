...
@Component({
  selector: 'app-group-event-form',
  templateUrl: './group-event-form.component.html',
  styleUrls: ['./group-event-form.component.scss']
})
export class GroupEventFormComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    await this.setOrganizationAddress();
    console.log(this.group_event);

    if (this.group_event.idGroupEvent) {
      this.help_request = this.group_event.help_request
      this.group_event.location = GoogleAddressComponent.formatFullAddress(this.group_event as any, ['street', 'city', 'state', 'zip_code', 'country']);

      const start_time = moment(this.group_event.full_start_date).format('HH:mm');
      const end_time = moment(this.group_event.full_end_date).format('HH:mm');
      console.log(start_time);
      console.log(end_time);
      this.group_event.event_time_start = start_time;
      this.group_event.event_time_end = end_time;
      if (this.group_event.start_date) {
        this.eventFormGroup.get('event_date_end').enable();
      }
      console.log(this.group_event.start_date);
      const start_date = moment.tz(this.group_event.full_start_date as string, 'YYYY-MM-DD', this.group_event.timezone);
      ...
      this.eventFormGroup.patchValue(
        {
          ...
          event_date_start: this.group_event.start_date,
          event_date_end: this.group_event.end_date,
          event_time_start: start_time,
          event_time_end: end_time,
          all_day: this.group_event.all_day
        }
      );
      ...
      if (this.group_event.is_exact_date) {
        ...
        this.eventFormGroup.get('event_current_week').setValue(moment(this.group_event.start_date).format('YYYY-MM-DD'));
      }
  ...
  async submitForm(form: FormGroup) {
    ...
    payload.start_date = `${payload.event_date_start} ${payload.event_time_start}`;
    payload.end_date = `${payload.event_date_end} ${payload.event_time_end}`;
    console.log(payload);
    ...
    // console.log(form)
    if (this.group_event.idGroupEvent) {
      this.group_event.start_date = payload.start_date;
      this.group_event.end_date = payload.end_date;
      subscription = this.groupService.updateEvent(this.group_event);
      message_error = `Error updating the event.`;
      message_success = `Event updated successfully.`;
    } else {
      subscription = this.groupService.addEvent(this.group_event);
...
`;
  ...
  formatEvents() {
    ...
    if (this.selected_frequency) {
      if (this.selected_frequency.is_exact_date) {
        ...
        const actual_event = moment(event_for_value.event_date)
      ...
      events
        ...
        .forEach((event) => {
          ...
          if (event.repeat_until_date) {
            ...
            last_day_of_end = moment(viewDateNext).endOf("month").toDate();
          }
  ...
  handleAllDay() {
    const is_all_day: boolean = this.eventFormGroup.get('all_day').value;
    if (is_all_day) {
      this.eventFormGroup.get('event_time_start').setValidators([]);
      this.eventFormGroup.get('event_time_end').setValidators([]);
    } else {
      this.eventFormGroup.get('event_time_start').setValidators([Validators.required]);
      this.eventFormGroup.get('event_time_end').setValidators([Validators.required]);
    }
  }
