...
@Component({
  selector: 'app-inventory-place-form',
  templateUrl: './inventory-place-form.component.html',
  styleUrls: ['./inventory-place-form.component.scss']
})
export class InventoryPlaceFormComponent implements OnInit {
  ...
  submitInventory(inventory_place_form: FormGroup) {
    ...
    if (inventory_place_form.valid) {
      ...
      if (payload.idInventoryPlace) {
        // update
        subscription = this.inventoryService.updateInventory(payload);
