...
@Component({
  selector: 'app-admin-design-request-form',
  templateUrl: './admin-design-request-form.component.html',
  styleUrls: ['./admin-design-request-form.component.scss']
})
export class DesignRequestFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    Promise.all(promises)
      .then((response: any) => {
        ...
        type.idDesignRequestPriority = 0;
        type.id = 0;
  ...
  validateInputs(designRequest: DesignRequestModel) {
    ...
    } else if (designRequest.idDesignRequestPriority === 0) {
      validate.message = 'You need to select a priority.';
