...
export class PipelineFormComponent implements OnInit {
  ...
  show_contact_home: boolean = false;
  show_organization_form: boolean = false;
  show_loader = true;

  contactsByIdIglesia: any[] = [];

  pipeline_form: FormGroup = this.form_builder.group({
    nombre: new FormControl('', [Validators.required]),
    idIglesia: new FormControl(undefined, [Validators.required]),
    idAgendaContactoIglesia: new FormControl(undefined, [Validators.required]),
  ...
  viewContacts(contact_home: ContactsHomeComponent) {
    this.show_form = false;
    this.show_contact_home = true;
    contact_home.iglesia = undefined;
    contact_home.iglesiaContactos = [];
    contact_home.iglesiaContactosOriginal = [];
    const iglesia = this.pipeline_form.get('idIglesia').value;
    if (iglesia) {
      if (iglesia.length > 0) {
        contact_home.iglesia = iglesia[0];
      } else {
        contact_home.iglesia = undefined;
      }
    } else {
